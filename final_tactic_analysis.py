#!/usr/bin/env python3
"""
Final correct tactic distribution analysis
- Sysmon: Events with Tactic != "Benign" are malicious
- NetFlow: Use Label column
"""
...
def analyze_sysmon(filepath):
    """Analyze Sysmon - events with Tactic != 'Benign' are malicious"""
    try:
        parts = Path(filepath).parts
        apt_type = parts[-2].split('-')[0] + '-' + parts[-2].split('-')[1]
        run_id = parts[-2].split('-')[-1]

        print(f"  Processing {apt_type}-{run_id} Sysmon...", flush=True)

        # Only read Tactic column
        df = pd.read_csv(filepath, usecols=['Tactic'], low_memory=False)

        total = len(df)

        # Events with Tactic != "Benign" are malicious
        malicious_df = df[(df['Tactic'].notna()) & (df['Tactic'] != 'Benign')]
        malicious = len(malicious_df)
        benign = total - malicious

        # Count tactics for malicious events
        tactic_counts = malicious_df['Tactic'].value_counts().to_dict()
        # Remove unwanted values
        tactic_counts = {str(k): v for k, v in tactic_counts.items() if str(k) not in ['nan', 'None', '', 'Benign']}

        print(f"    -> {total} total, {malicious} malicious", flush=True)
        return apt_type, run_id, 'Sysmon', total, benign, malicious, tactic_counts
    except Exception as e:
        print(f"Error with {filepath}: {e}", flush=True)
        return None

def analyze_netflow(filepath):
    """Analyze NetFlow - use Label column"""
    try:
        parts = Path(filepath).parts
        apt_type = parts[-2].split('-')[0] + '-' + parts[-2].split('-')[1]
        run_id = parts[-2].split('-')[-1]

        print(f"  Processing {apt_type}-{run_id} NetFlow...", flush=True)
        ...
        print(f"    -> {total} total, {malicious} malicious", flush=True)
        return apt_type, run_id, 'NetFlow', total, benign, malicious, tactic_counts
    except Exception as e:
        print(f"Error with {filepath}: {e}", flush=True)
        return None

# Find files
print("Finding files...", flush=True)
...
print(f"Found {len(sysmon_files)} Sysmon + {len(netflow_files)} NetFlow datasets\n", flush=True)

# Process all
results = []

print("Processing Sysmon datasets...", flush=True)
for i, f in enumerate(sysmon_files, 1):
    print(f"[{i}/{len(sysmon_files)}]", end=" ", flush=True)
    r = analyze_sysmon(f)
    if r: results.append(r)

print("\nProcessing NetFlow datasets...", flush=True)
for i, f in enumerate(netflow_files, 1):
    print(f"[{i}/{len(netflow_files)}]", end=" ", flush=True)
    r = analyze_netflow(f)
    if r: results.append(r)
...
print(f"Found tactics: {all_tactics}\n", flush=True)
