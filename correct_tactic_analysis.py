#!/usr/bin/env python3
"""
Correct tactic distribution analysis
- Sysmon: Any event with a Tactic is malicious
...
def analyze_sysmon(filepath):
    """Analyze Sysmon - events with Tactic are malicious"""
    try:
        ...
        # Events with non-null Tactic are malicious
        malicious_df = df[df['Tactic'].notna()]
        malicious = len(malicious_df)
        benign = total - malicious

        # Count tactics for malicious events
        tactic_counts = malicious_df['Tactic'].value_counts().to_dict()
        # Remove 'nan' string if present
        tactic_counts = {str(k): v for k, v in tactic_counts.items() if str(k) not in ['nan', 'None', '']}
